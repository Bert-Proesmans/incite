...
#[async]
pub fn setup_server(control: ServerConfig) -> Result<(TcpListener, SharedLobbyState)> {
    let ServerConfig {
        application_logger,
        bind_address,
        ..
...
#[async]
pub fn handle_connections(server: TcpListener, shared_state: SharedLobbyState) -> Result<()> {
